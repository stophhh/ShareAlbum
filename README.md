...
Momento는 친구, 가족, 여행 멤버가 함께 사진을 업로드하고 댓글과 반응을 남길 수 있는 공유 앨범 앱입니다. 초대코드로 앨범에 참여할 수 있고, 스마트 검색 탭에서 “올해 사진 보여줘”, “여름 사진으로 앨범 만들어줘”처럼 자연어에 가까운 명령으로 사진을 찾거나 새 앨범을 만들 수 있습니다.

현재 버전은 OpenAI 비용 문제를 고려해 실제 LLM/Vision 호출 대신 FastAPI 기반 Demo Agent로 구현했습니다. Android 앱이 현재 앨범의 사진 URL과 자동 생성 태그를 FastAPI 서버로 전달하면, 서버가 날짜, 계절, 업로더 등 기본 메타데이터를 기준으로 사진을 검색하고 결과를 반환합니다. 이후 OpenAI Vision/Embedding을 연결하면 사진 내용 기반 자동 태깅과 의미 검색으로 확장할 수 있도록 구조를 분리했습니다.
...
Momento는 공유 앨범과 자연어 기반 검색 흐름을 결합해, 사진을 함께 모으고 나중에 더 쉽게 다시 찾는 경험을 목표로 했습니다.
